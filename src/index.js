...
(async () => {
  ...
  try {
    ...
    Options.auth.authorizationCode = async () => Client.consoleQuestion('[LedxBot] [FORTNITE] Please enter an authorization code: ');
  }
  ...
  if (config.discord.token === 'TOKEN') return console.log('[LedxBot] [DISCORD]', 'Please enter a valid token in config.js');
  ...
  bot.on('party:member:joined', member => {
    ...
    console.log('[LedxBot] [FORTNITE]', `${member.displayName} has joined the party. New member count: ${bot.party.members.size}.`);
  ...
  bot.on('party:invite', invite => {
    console.log('[LedxBot] [FORTNITE]', `Received a party invite from ${invite.sender.displayName}.`);
    ...
    console.log('[LedxBot] [FORTNITE]', `Invite from ${invite.sender.displayName} has been ${config.fortnite.acceptInvite ? 'accepted' : 'declined'}.`);
  });

  bot.on('friend:request', request => {
    console.log('[LedxBot] [FORTNITE]', `Received a friend request from ${request.displayName}.`);
    ...
    console.log('[LedxBot] [FORTNITE]', `Friend request from ${request.displayName} has been ${config.fortnite.acceptFriend ? 'accepted' : 'declined'}.`);
  });
  ...
  client.on('ready', () => {
    ...
    console.log('[LedxBot] [DISCORD]', `Client ready as ${client.user.tag} [${client.commands.size} commands]`);
    console.log('[LedxBot] [FORTNITE]', `Client ready as ${bot.user.displayName}.`);
  });
})();
